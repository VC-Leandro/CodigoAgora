jQuery(document).ready(function ($) {
  ...
  $('li#toplevel_page_emsb_admin_page .dashicons-admin-generic').append(
    "<span class='emsb-icon-wrapper'><img src='" +
      emsb_icon_url +
      "' alt='EMSB'></span>"
  );
  ...
  $(document).on('click', '.emsb-table-wrapper table tr', function () {
    ...
    $(this)
      .children('.emsb-pending-order-checkbox')
      .children('input')
      .prop('checked', true);
    $(this)
      .siblings()
  ...
  $(document).on(
    'click',
    '.emsb-active-pending-order .emsb-approval-action',
    function () {
      ...
      var emsb_booking_approval_action_value = $(this)
        .children('.emsb-booking-action-value')
        .val();
      var emsb_booking_update_availability = $(this)
        .children('.emsb-booking-update-avalability')
        .val();
      var booked_slot_id = $(this).children('.emsb-booked-slot-id').val();
      var emsb_customer_email_address = $(this)
        .children('.emsb-customer-email-address')
        .val();
      var emsb_booking_approval_id = $(this)
        .children('.emsb-booking-approval-id')
        .val();
      var emsb_booking_approval_action_data = {
        ...
        emsb_booking_update_availability: emsb_booking_update_availability + 1,
  ...
  jQuery('#toplevel_page_emsb_admin_page .wp-menu-name').append(
    "<span class='emsb-pending-bookings-count'></span>"
  );
  ...
  function preparePendingBookingTable(data) {
    ...
    $.each(data, function (index, booking) {
      ...
      var update_availability = availability_per_slot_to_int;
      var do_not_update_availability = parseInt(
        availability_per_slot_to_int + 1
      );
      ...
      $('#emsbPendingBookings').append(
        "\
          ...
          "  </td>\
                    <td> \
                        <div class='emsb-booking-status-wrapper'> \
                            <span> Aguardando Confirmação </span> <br>" +
          availability_per_slot_to_int +
          " </span>\
                            <div class='emsb-approval-actions-wrapper'>\
                                <a class='emsb-approval-action emsb-approval-approve'>\
                                    <input class='emsb-booking-approval-id' type='hidden' value='" +
          booking.id +
          "' name='emsb_booking_approval_id' >\
                                    <input class='emsb-booking-action-value' type='hidden' value='1' name='emsb_booking_approved' > Confirmar \
                                    <input class='emsb-booking-update-avalability' type='hidden' value='" +
          update_availability +
          "' >\
                                    <input class='emsb-booked-slot-id' type='hidden' value='" +
          booking.booked_slot_id +
          "' >\
                                    <input class='emsb-customer-email-address' type='hidden' value='" +
          booking.customer_email +
          "' >\
                                </a>\
          ...
          booking.id +
          "' name='emsb_booking_approval_id' >\
                                    <input class='emsb-booking-action-value' type='hidden' value='trush' name='emsb_booking_trush' > Cancelar \
                                    <input class='emsb-booking-update-avalability' type='hidden' value='" +
          do_not_update_availability +
          "' >\
                                    <input class='emsb-customer-email-address' type='hidden' value='" +
          booking.customer_email +
          "' >\
  ...
  function fetchPendingBookingsCounts() {
    ...
    $.ajax({
      ...
      success: function (result) {
        jQuery(
          '#toplevel_page_emsb_admin_page .wp-menu-name .emsb-pending-bookings-count'
        ).text(result);
      },
    });
  }
