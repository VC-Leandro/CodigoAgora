jQuery( document ).ready(function( $ ) {
    ...
    function addServiceDateId(){
        ...
        var i = 1;
        for(i=1;i<=31; i++){
            $(".em-reservation-calendar tbody.month-days tr td[data-date='" + i +"']").attr('data-servicedateid', emsb_selected_service_each_date_id +'-'+ i).attr('title', 'Indisponível').addClass("unavailable emsb-service-date-unavailable").attr('disabled', true).attr('data-toggle', 'tooltip');
    ...
    function disableOffDays(){
        ...
        jQuery.each( offDays, function( i, val ) {
            offDay = $(".em-reservation-calendar tbody.month-days tr").children('td:nth-child('+ val +')').addClass("unavailable off-day").attr('disabled', true).attr('title', 'Indisponível');
    ...
    function disablePassedDays(){
        ...
        if(currentYear === activeYearValue && currentMonth === activeMonthValue){
            ...
            for(i=1;i<todayDateValue; i++){
                $(".em-reservation-calendar tbody.month-days tr td[data-date='" + i +"']").addClass("unavailable passed-day").attr('disabled', true).attr('title', 'Data Indisponível');
            }
        } 
    ...
    function onIcClickdisablePassedDays(){
        ...
        if(currentYear == activeYearValue && currentMonth == activeMonthValue){
            for(i=1;i<todayDateValue; i++){
                $(".em-reservation-calendar tbody.month-days tr td[data-date='" + i +"']").addClass("unavailable passed-day").removeClass("available emsb-service-date-available").attr('disabled', true).attr('title', 'Data Indisponível');
            }
            
        } else if(currentYear == activeYearValue  && currentMonth > activeMonthValue){
            todayDateValue = 31;
            for(i;i<todayDateValue; i++){
                $(".em-reservation-calendar tbody.month-days tr td").addClass("unavailable passed-day").removeClass("available emsb-service-date-available").attr('disabled', true).attr('title', 'Data Indisponível');
            }
    ...
    function enableDatesAsAvailableOnlyOnTimeSlot(){   
        ...
        for(var i=1;i<=31; i++){
            ...
            var makeSmallerCDate = datesOnCalendarGetDateTime / 1000;
            if(makeSmallerStartingDate <= makeSmallerCDate && makeSmallerCDate <= makeSmallerEndingDate){
                $(".em-reservation-calendar tbody.month-days tr td[data-date='" + i +"']").attr('title', 'Disponível').removeClass("unavailable emsb-service-date-unavailable").addClass("emsb-service-date-available"); 
            }
       ...
       // Disable the already booked dates with ajax
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if(response == 'error'){
                    $(".em-reservation-calendar tbody.month-days tr td").addClass("unavailable already-booked").attr('title', 'Horário Indisponível');
                } else {
                    $.each(response, function(index, slot) {
                        ...
                        if(!isOffDay && !isDatePassed && !isunavailable){ 
                            if(slot.available_orders == 0){
                                $(".em-reservation-calendar tbody.month-days tr td[data-servicedateid='" + slot.booked_date_id +"']").addClass("unavailable already-booked").attr('title', 'Horário Indisponível');
                            } else {                      
        ...
        $.ajax({
            ...
            success: function(emsb_slot_response) {
                ...
                if(emsb_slot_response == 'error'){
                    $(".slots li button").addClass("booked").removeClass("available").attr('title', 'Horário Indisponível').text("Horário Indisponível");
                } else {
                    $.each(emsb_slot_response, function(index, slot) {
                        ...
                        if(starting_time_ms > currentTimeInMiliSeconds){
                            if(availableOrdersInInt <= 0){
                                $(".slots li[data-slotid='" + slot.booked_slot_id +"'] button").addClass("booked").removeClass("available").attr('title', 'Horário Indisponível').text("Horário Indisponível");
                            } else {
                                $(".slots li[data-slotid='" + slot.booked_slot_id +"'] button").text("Horário Disponível").attr('title', "Horário Disponível: "+ slot.available_orders);
                            }
                        } else {
                            $(".slots li[data-slotid='" + slot.booked_slot_id +"'] button").text("Horário Indisponível").attr('title', "Horário Indisponível").addClass("booked").removeClass("available");
                        }
